                ...
                for key, val in kw.items():
                    if isinstance(val, (list, tuple)):
                        ...
                        if val is None:
                            continue
                        query_args.append(
                            "{}={}".format(quote(str(key)), quote(str(val)))
                        )
                if query_args:
                    my_url += "?"
...
def _link_active(kwargs: Any) -> bool:
    ...
    return (
        kwargs.get("controller") == blueprint
        and kwargs.get("action") == endpoint
    )
...
def _url_with_params(
    url: str, params: Optional[Iterable[tuple[str, Any]]]
) -> str:
    if not params:
        return url
    params = [
...
@core_helper
def sorted_extras(
    package_extras: list[dict[str, Any]],
...
@core_helper
def check_access(
    action: str, data_dict: Optional[dict[str, Any]] = None
) -> bool:
...
@core_helper
def linked_user(
    user: Union[str, model.User], maxlength: int = 0, avatar: int = 20
) -> Union[Markup, str, None]:
...
@core_helper
@maintain.deprecated(
    "helpers.truncate() is deprecated and will be removed "
...
def markdown_extract(
    text: str, extract_length: int = 190
) -> Union[str, Markup]:
    ...
    return literal(
        str(shorten(plain, width=extract_length, placeholder="..."))
    )


@core_helper
def dict_list_reduce(
    list_: list[dict[str, T]], key: str, unique: bool = True
) -> list[T]:
...
@core_helper
def gravatar(
    email_hash: str, size: int = 100, default: Optional[str] = None
) -> Markup:
    if default is None:
...
@core_helper
def get_page_number(
    params: dict[str, Any], key: str = "page", default: int = 1
) -> int:
    """
    ...
    try:
        ...
        base.abort(
            400,
            ('"{key}" parameter must be a positive integer'.format(key=key)),
        )
...
def date_str_to_datetime(date_str: str) -> datetime.datetime:
    ...
    if len(time_tuple) >= 6:
        m = re.match(
            r"(?P<seconds>\d{2})(\.(?P<microseconds>\d+))?$", time_tuple[5]
        )
        if not m:
            raise ValueError(
        ...
        microseconds = int(
            (str(m.groupdict("0")["microseconds"]) + "00000")[0:6]
        )
...
def parse_rfc_2822_date(
    date_str: str, assume_utc: bool = True
...
@core_helper
def popular(
    type_: str, number: int, min: int = 1, title: Optional[str] = None
) -> str:
    ...
    return snippet(
        "snippets/popular.html", title=title, number=number, min=min
    )
...
def organizations_available(
    permission: str = "manage_group", include_dataset_count: bool = False
) -> list[dict[str, Any]]:
...
def get_pkg_dict_extra(
    pkg_dict: dict[str, Any], key: str, default: Optional[Any] = None
) -> Any:
...
def get_request_param(
    parameter_name: str, default: Optional[Any] = None
) -> Any:
...
def format_resource_items(
    ...
    for key, value in items:
        if key in blacklist or (
            not isinstance(value, (int, float)) and not value
        ):
...
def get_allowed_view_types(
    resource: dict[str, Any], package: dict[str, Any]
...
def get_organization(
    org: Optional[str] = None, include_datasets: bool = False
) -> dict[str, Any]:
    if org is None:
        return {}
    try:
...
def get_translated(data_dict: dict[str, Any], field: str) -> Union[str, Any]:
    ...
    try:
        ...
        if base_language != language:
            return data_dict[field + "_translated"][base_language]
    except KeyError:
        pass
...
def facets() -> list[str]:
    ...
    return config.get("search.facets")
...
def load_plugin_helpers() -> None:
    ...
    for name, func_list in chained_helpers.items():
        ...
        for func in reversed(func_list):
            new_func = functools.partial(func, helper_functions[name])
...
def get_collaborators(package_id: str) -> list[tuple[str, str]]:
    ...
    """
    context: Context = {"ignore_auth": True, "user": current_user.name}
    ...
    for collaborator in _collaborators:
        collaborators.append(
            (collaborator["user_id"], collaborator["capacity"])
        )
...
def can_update_owner_org(
    ...
    user_orgs: Optional[list[dict[str, Any]]] = None,
) -> bool:
...
def decode_view_request_filters() -> dict[str, Any] | None:
    ...
    if filterString is not None and len(filterString) > 0:
        filters = {}
        for k_v in filterString.split("|"):
            k, _sep, v = k_v.partition(":")
...
def check_ckan_version(
    ...
    """
    return p.toolkit.check_ckan_version(
