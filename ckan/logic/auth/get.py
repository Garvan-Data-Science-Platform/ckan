...
def admin(context: Context, data_dict: DataDict) -> AuthResult:
...
def resource_show(context: Context, data_dict: DataDict) -> AuthResult:
    ...
    pkg_dict = {"id": pkg.id}
    authorized = authz.is_authorized("package_show", context, pkg_dict).get(
        "success"
    )
...
def group_show(context: Context, data_dict: DataDict) -> AuthResult:
    ...
    if group.state == "active":
        if config.get("ckan.auth.public_user_details") or (
            not asbool(data_dict.get("include_users", False))
...
def api_token_list(context: Context, data_dict: DataDict) -> AuthResult:
    ...
    user = context["model"].User.get(id_or_name)
...
def package_collaborator_list(
    ...
    if not authz.can_manage_collaborators(pkg.id, user_obj.id):
        return {
            "success": False,
            "msg": _(
                "User %s not authorized to list collaborators from this dataset"
            )
            % user,
        }
