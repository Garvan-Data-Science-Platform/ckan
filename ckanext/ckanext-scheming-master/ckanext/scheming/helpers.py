...
from ckantoolkit import config, _, get_action, ObjectNotFound
...
def make_fn(tag):
    def _fn(arg):
        create_vocab(tag["vocabulary"], tag["tags"])
        try:
            tag_list = get_action("tag_list")
            tl = tag_list({}, {"vocabulary_id": tag["vocabulary"]})
            return [{"label": t, "value": t} for t in tl]
        except ObjectNotFound:
            return []
...
for tag in tags:
    all_helpers[tag["vocabulary"]] = make_fn(tag)
...
def scheming_datastore_choices(field):
    ...
    datastore_choices = [
        {"value": r[fields[0]], "label": r[fields[1]]}
...
def date_tz_str_to_datetime(date_str):
    ...
    if len(time_tuple) >= 6:
        m = re.match(
            "(?P<seconds>\d{2})(\.(?P<microseconds>\d{3,6}))?$", time_tuple[5]
        )
        if not m:
            raise ValueError(
