...
class _SchemingMixin(object):
    ...
    def _load_presets(config):
        ...
        presets = reversed(
            config.get("scheming.presets", DEFAULT_PRESETS).split()
        )
    ...
    def update_config(self, config):
        ...
        self._schemas = _load_schemas(
            self._schema_urls, self.SCHEMA_TYPE_FIELD
        )
...
class SchemingDatasetsPlugin(
    p.SingletonPlugin, DefaultDatasetForm, _SchemingMixin
):
    ...
    def dataset_facets(self, facets_dict, package_type):
        ...
        for tag in tags:
            facets_dict[tag["vocabulary"]] = p.toolkit._(tag["label"])
...
def _expand_schemas(schemas):
    ...
    for name, original in schemas.items():
        ...
        for grouping in ("fields", "dataset_fields", "resource_fields"):
            ...
            schema[grouping] = [
                _expand(schema, field) for field in schema[grouping]
            ]
